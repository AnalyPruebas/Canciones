...
export class CancionController {
  ...
  async eliminar(@Res() response, @Param('idCancion') idCancion: string) {
    const cancion = await this._cancionService.buscarPorId(+idCancion);

    await this._cancionService.eliminar(Number(idCancion));
    const parametrosConsulta = `?accion=borrar&titulo=${cancion.nombre}`;
    response.redirect('cancion/inicio' + parametrosConsulta);
  }

  @Get('crear-cancion')
  crearcancionRuta(@Res() response) {
    response.render('crear-cancion');
  }

  @Post('crear-cancion')
  crearcancionFuncion(@Res() response, @Body() cancion: Cancion) {
